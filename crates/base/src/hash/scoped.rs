...
//! Taken from `gluon/base/src/scoped_map.rs`.
use std::{
    borrow::Borrow,
    collections::hash_map::{self, IterMut},
    fmt,
    hash::Hash,
};
...
impl<K: Eq + Hash + Clone, V> ScopedMap<K, V> {
    ...
    /// Exits the current scope, returning an iterator over the (key, value) pairs that are
    /// removed.
    ...
    pub fn pop(&mut self, k: &K) -> Option<V> {
        match self.map.get_mut(k).and_then(|x| x.pop()) {
            Some(v) => {
                let mut i = self.scopes.len() as isize - 1;
                while i >= 0 {
                    if self.scopes[i as usize].as_ref() == Some(k) {
                        self.scopes.remove(i as usize);
                    }
                    i -= 1;
                }
...
impl<K, V> Drop for ExitScopeIter<'_, K, V>
where
    K: Eq + Hash + Clone,
{
    fn drop(&mut self) {
...
impl<K, V> Iterator for ExitScopeIter<'_, K, V>
where
    K: Eq + Hash + Clone,
{
    type Item = (K, V);
