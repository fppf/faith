...
use crate::index::{Idx, IndexSlice, IndexVec};
...
// TODO. would be nice to have inside `newtype_index`.
impl std::iter::Step for PreorderIndex {
    #[inline]
    fn steps_between(start: &Self, end: &Self) -> (usize, Option<usize>) {
        <usize as std::iter::Step>::steps_between(&Self::index(*start), &Self::index(*end))
    }
