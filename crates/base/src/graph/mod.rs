//! A graph module for use in dataflow and elsewhere.
// Taken from `rustc_data_structures/src/graph`.
use std::fmt::Debug;

use crate::{bitset::BitSet, index::Idx};
...
{
    iter::TriColorDepthFirstSearch::new(graph)
        .run_from_start(&mut iter::CycleDetector)
        .is_some()
}

pub fn depth_first_search<G>(graph: G, from: G::Node) -> iter::DepthFirstSearch<G>
where
    G: Successors,
{
    iter::DepthFirstSearch::new(graph).with_start_node(from)
}

pub fn depth_first_search_as_undirected<G>(
    graph: G,
    from: G::Node,
) -> iter::DepthFirstSearch<impl Successors<Node = G::Node>>
where
    G: Successors + Predecessors,
{
    ...
    iter::DepthFirstSearch::new(AsUndirected(graph)).with_start_node(from)
}
...
impl<N: Debug, E: Debug> Iterator for DepthFirstTraversal<'_, N, E> {
    type Item = NodeIndex;
...
impl<N: Debug, E: Debug> ExactSizeIterator for DepthFirstTraversal<'_, N, E> {}
